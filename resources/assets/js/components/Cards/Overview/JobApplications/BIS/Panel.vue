<template>
    <div class="col" id="job_applications_bis_col">
        <!-- print markup -->
        ...
        <div class="m-portlet d-print-none">
            <div class="m-portlet__head">
                ...
                <div class="m-portlet__head-tools">
                    <ul class="m-portlet__nav">
                        ...
                        <li class="m-portlet__nav-item">
                            <a @click="isOpen()" class="m-portlet__nav-link m-portlet__nav-link--icon" data-toggle="collapse" href="#BISJobApp" role="button" aria-expanded="false" aria-controls="BISJobApp">
            ...
            <div class="m-portlet__body collapse table-responsive" id="BISJobApp">
                ...
                <!--/desktop markup -->
                <!-- mobile (swipe) markup -->
                <div id='job_applications_bis_slider' class='swipe' v-else-if="panel.loading == 0 && isMobile">
                    <div class='swipe-wrap'>
                        <div class='swipe-card' v-for="item in panel.data">
                            ...
                            <div class="row" v-if="item.applicant">
                                <p class="col">
                                    <strong class="m--font-boldest">
                                        Applicant
                                    </strong>
...
<script>
	...
	import BISJobAppNum from './Columns/JobAppNum.vue'
	...
	Vue.component('bis-job-app-num', BISJobAppNum)
	Vue.component('bis-job-view-more', ViewMore)
    Vue.component('bis-job-status', Status)
	...
	export default { 
		...
		data () { 
			return { 
				fields: [ 
					{ 
						name: '__component:bis-job-view-more', 
						sortField: '', 
						title: 'View'
					},
					{ 
					...
					{ 
						name: '__component:bis-job-status', 
						sortField: '', 
				...
				css: {
			        table: {
			        	tableClass: 'table table-striped table-hover custom-panel bis-job-applications-panel',
			    ...
			    sortOrder: [
			        { name: 'file_date', sortField: 'file_date', direction: 'asc' },
			    ],
			    moreParams: {},
		...
		mounted() {
		    if (!this.isMobile) this.$events.$on('bis-job-app-filter-set', eventData => this.onFilterSet(eventData))
		    if (!this.isMobile) this.$events.$on('bis-job-app-filter-reset', e => this.onFilterReset())
		},
		watch: {
			'property': 'loadApiUrl',
		...
		methods: {
            ...
            openMobilePanel () {
                ...
                document.getElementById('BISJobApp').style = 'min-height: calc(100vh - 186.72px);'
                document.getElementById('job_applications_bis_col').style = 'padding: 0px; position: fixed; top: 71.13px; height: 100%'
            ...
            closeMobilePanel () {
                ...
                document.getElementById('BISJobApp').style = 'min-height: \'\';'
                document.getElementById('job_applications_bis_col').style = 'padding: 15px; position: \'\'; top: \'\'; height: \'\';'
                document.getElementById('row_parent').style = 'padding: 0px 30px;'
            },
            getMobileDataOnPageLoad () {
                axios.get(`/api/property/bis/job-applications/swipe/${this.property.id}`)
                    .then((response) => {
			...
			loadApiUrl () {
				var app = this
				app.panel.api_url = `/property/bis/job-applications/open/${this.property.id}`
            ...
            applySwipe () {
                const swipeContainer = document.getElementById('job_applications_bis_slider')
            ...
            getMobileData () {
                const app = this
                axios.get(`/api/property/bis/job-applications/swipe/${app.property.id}`)
                    .then((response) => {
			...
			updateVueTable () {
				...
				$('#BISJobApp').collapse("hide")
			},
			viewOpen (id) {
				var app = this
				app.panel.api_url = `/property/bis/job-applications/open/${id}`
				app.$refs.vuetable.refresh()
				app.panel.closed_items = 0
				app.panel.all_items = 0
				app.panel.open_items = 1
				$('#BISJobApp').collapse("show")
			},

			viewClosed (id) {
				var app = this
				app.panel.api_url = `/property/bis/job-applications/closed/${id}`
				app.$refs.vuetable.refresh()
				app.open_items = 0
				app.closed_items = 0
			...
			viewAll (id) {
				var app = this
				app.panel.api_url = `/property/bis/job-applications/all/${id}`
				app.$refs.vuetable.refresh()
				app.open_items = 0
				app.closed_items = 0
...
<style>
    .bis-job-applications-panel .LatestActionDateHeader, 
    .bis-job-applications-panel .LatestActionDateData, 
    .bis-job-applications-panel .StatusHeader, 
    .bis-job-applications-panel .StatusData { 
        min-width: 150px !important;
    }

    .bis-job-applications-panel .DocNumberHeader, 
    .bis-job-applications-panel .DocNumberData { 
