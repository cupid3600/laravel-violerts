<template>
    <div class="col" id="job_applications_open_data_col">
        <!-- print markup -->
        ...
        <div class="m-portlet d-print-none">
            <div class="m-portlet__head">
                ...
                <div class="m-portlet__head-tools">
                    <ul class="m-portlet__nav">
                        ...
                        <li class="m-portlet__nav-item">
                            <a @click="isOpen()" class="m-portlet__nav-link m-portlet__nav-link--icon" data-toggle="collapse" href="#OpenDataJobApp" role="button" aria-expanded="false" aria-controls="OpenDataJobApp">
            ...
            <div class="m-portlet__body collapse table-responsive" id="OpenDataJobApp">
                ...
                <vuetable
                        ...
                        :api-url="`/property/open-data/job-applications/${property.id}`"
                        :fields="fields"
                        :css="css.table"
                        :sortOrder="sortOrder"
                ...
                <!-- desktop markup -->
                <!-- mobile (swipe) markup -->
                <div id="job_applications_open_data_slider" class="swipe" v-else-if="panel.loading == 0 && isMobile">
...
<script>
	...
	import JobAppWorkType from './Columns/ODJobWorkType.vue'
	import JobAppApplicant from './Columns/ODJobApplicant.vue'
	import LatestActionDate from './Columns/LatestActionDate.vue'
	...
	Vue.component('job-app-work-type', JobAppWorkType)
	Vue.component('job-app-applicant', JobAppApplicant)
	Vue.component('od-lastest-action-date', LatestActionDate)
	Vue.component('od-job-app-view-more', ViewMore)
	Vue.component('od-job-app-status', Status)
	...
	export default { 
		...
		data () { 
			return { 
				fields: [ 
					{ 
						name: '__component:od-job-app-view-more', 
						sortField: '', 
						title: 'View'
					}, 
					{ 
						name: '__component:od-lastest-action-date', 
						sortField: '', 
						headerClass: 'LatestActionDateHeader', 
						dataClass: 'LatestActionDateData',
					...
					{ 
						name: 'doc__', 
						sortField: '', 
						headerClass: 'DocNumberHeader', 
						dataClass: 'DocNumberData',
						title: 'Doc #'
					},
					{ 
						name: 'job_application_ext.job_description', 
						sortField: '', 
						title: 'Description'
					},
					{ 
						name: '__component:job-app-applicant', 
						sortField: '',
						headerClass: 'ApplicantHeader', 
						dataClass: 'ApplicantData',
						title: 'Applicant'
					},
					{ 
						name: '__component:od-job-app-status', 
						sortField: '', 
						headerClass: 'StatusHeader', 
						dataClass: 'StatusData',
						title: 'Status'
					},
				],
				css: {
			        table: {
			        	tableClass: 'table table-striped table-hover custom-panel bis-job-applications',
		...
		mounted() {
		    if(!this.isMobile) this.$events.$on('job-app-filter-set', eventData => this.onFilterSet(eventData))
		    if(!this.isMobile) this.$events.$on('job-app-filter-reset', e => this.onFilterReset())
		},
		watch: {
		...
		methods: {
            ...
            openMobilePanel () {
                ...
                document.getElementById('OpenDataJobApp').style = 'min-height: calc(100vh - 186.72px);'
                document.getElementById('job_applications_open_data_col').style = 'padding: 0px; position: fixed; top: 71.13px; height: 100%'
            ...
            closeMobilePanel () {
                ...
                document.getElementById('OpenDataJobApp').style = 'min-height: \'\';'
                document.getElementById('job_applications_open_data_col').style = 'padding: 15px; position: \'\'; top: \'\'; height: \'\';'
                document.getElementById('row_parent').style = 'padding: 0px 30px;'
            },
            getMobileDataOnPageLoad () {
                axios.get(`/api/property/open-data/job-applications/swipe/${this.property.id}`)
                    .then((response) => {
            ...
            getMobileData () {
                const app = this
                axios.get(`/api/property/open-data/job-applications/swipe/${app.property.id}`)
                    .then((response) => {
            ...
            applySwipe () {
                const swipeContainer = document.getElementById('job_applications_open_data_slider')
...
<style>
	.bis-job-applications .LatestActionDateHeader, .bis-job-applications .LatestActionDateData { 
		width: 150px !important;
	}

	.bis-job-applications .DocNumberHeader, .bis-job-applications .DocNumberData { 
		width: 70px !important;
	}

	.bis-job-applications .ApplicantHeader, .bis-job-applications .ApplicantData { 
		width: 150px !important;
	}

	.bis-job-applications .StatusHeader, .bis-job-applications .StatusData { 
	...
	#job_applications_open_data_col {
