<template>
	<div class="col" id="bis_elevator_violations_col">
		<div class="m-portlet">
			<div class="m-portlet__head">
				...
				<div class="m-portlet__head-tools">
					<ul class="m-portlet__nav">
						...
						<li class="m-portlet__nav-item">
							<a @click="isOpen()" class="m-portlet__nav-link m-portlet__nav-link--icon" data-toggle="collapse" href="#BISElevatorViolations" role="button" aria-expanded="false" aria-controls="BISElevatorViolations">
			...
			<div class="m-portlet__body collapse table-responsive" id="BISElevatorViolations">
				...
				<vuetable
					...
					:api-url="`/property/bis/elevators/violations/${property.id}`"
					:fields="fields"
			        :css="css.table"
			        :sortOrder="sortOrder"
				...
				<div id="bis_elevator_violations_slider" class="swipe" v-else-if="panel.loading == 0 && isMobile">
			...
			<div class="m-portlet__foot" v-if="panel.loading == 0">
				<div class="row align-items-center">
					<div class="col-lg-6 m--valign-middle">
						<div v-if="panel.show_page_count">
							<p>Slide {{ panel.current_slide }} of {{ panel.data.length }}</p>
						</div>
...
<script>
	...
	export default { 
		...
		data () { 
			return { 
				fields: [ 
					...
					{ 
						name: 'disposition_badge', 
						sortField: '', 
						title: 'Disposition Badge'
					},
				],
				css: {
			        table: {
		...
		mounted() {
		    if (!this.isMobile) this.$events.$on('bis-elevator-violations-filter-set', eventData => this.onFilterSet(eventData))
		    if (!this.isMobile) this.$events.$on('bis-elevator-violations-filter-reset', e => this.onFilterReset())
		},
		watch: { 
		...
		methods: {
            ...
            openMobilePanel () {
                ...
                document.getElementById('BISElevatorViolations').style = 'min-height: calc(100vh - 186.72px);'
                document.getElementById('bis_elevator_violations_col').style = 'padding: 0px; position: fixed; top: 71.13px; height: 100%'
            ...
            closeMobilePanel () {
                ...
                document.getElementById('BISElevatorViolations').style = 'min-height: \'\';'
                document.getElementById('bis_elevator_violations_col').style = 'padding: 15px; position: \'\'; top: \'\'; height: \'\';'
                document.getElementById('row_parent').style = 'padding: 0px 30px;'
            },
            getMobileDataOnPageLoad () {
                axios.get(`/api/property/bis/elevator/violations/swipe/${this.property.id}`)
                    .then((response) => {
            ...
            applySwipe () {
                const swipeContainer = document.getElementById('bis_elevator_violations_slider')
            ...
            getMobileData () {
                const app = this
                app.isLoading()
                axios.get(`/api/property/bis/elevator/violations/swipe/${app.property.id}`)
                    .then((response) => {
                        this.panel.data = response.data
                        app.isLoading()
			...
			updateVueTable () {
				...
				$('#BISElevatorViolations').collapse("hide")
			},
			getInspectionData () { 
				var app = this
