<template>
	<div class="col" id="bis_illuminated_signs_col">
		<div class="m-portlet">
			<div class="m-portlet__head">
				...
				<div class="m-portlet__head-tools">
					<ul class="m-portlet__nav">
						...
						<li class="m-portlet__nav-item">
							<a @click="isOpen()" class="m-portlet__nav-link m-portlet__nav-link--icon" data-toggle="collapse" href="#BISIlluminatedSigns" role="button" aria-expanded="false" aria-controls="BISIlluminatedSigns">
			...
			<div class="m-portlet__body collapse table-responsive" id="BISIlluminatedSigns">
				...
				<vuetable
					...
					:api-url="`/property/bis/illuminated-signs/${property.id}`"
					:fields="fields"
			        :css="css.table"
			        :sortOrder="sortOrder"
				...
				<div id='bis_illuminated_signs_slider' class='swipe' v-else-if="panel.loading == 0 && isMobile">
...
<script>
	...
	export default {
		...
		data () { 
			return { 
				fields: [ 
					...
					{ 
						name: 'last_permit_issued', 
						sortField: '', 
						title: 'Last Permit Issued'
					},
					{ 
						name: 'permit_code', 
						sortField: '', 
		...
		mounted() {
		    if (!this.isMobile) this.$events.$on('bis-illuminated-signs-filter-set', eventData => this.onFilterSet(eventData))
		    if (!this.isMobile) this.$events.$on('bis-illuminated-signs-filter-reset', e => this.onFilterReset())
		},
		watch: { 
		...
		methods: {
            ...
            openMobilePanel () {
                ...
                document.getElementById('BISIlluminatedSigns').style = 'min-height: calc(100vh - 186.72px);'
                document.getElementById('bis_illuminated_signs_col').style = 'padding: 0px; position: fixed; top: 71.13px; height: 100%'
            ...
            closeMobilePanel () {
                ...
                document.getElementById('BISIlluminatedSigns').style = 'min-height: \'\';'
                document.getElementById('bis_illuminated_signs_col').style = 'padding: 15px; position: \'\'; top: \'\'; height: \'\';'
                document.getElementById('row_parent').style = 'padding: 0px 30px;'
            },
            getMobileDataOnPageLoad () {
                axios.get(`/api/property/bis/illuminated-signs/swipe/${this.property.id}`)
                    .then((response) => {
            ...
            applySwipe () {
                const swipeContainer = document.getElementById('bis_illuminated_signs_slider')
            ...
            getMobileData () {
                const app = this
                app.isLoading()
                axios.get(`/api/property/bis/illuminated-signs/swipe/${app.property.id}`)
                    .then((response) => {
                        this.panel.data = response.data
                        app.isLoading()
			...
			getData () { 
				var app = this
				app.isLoading()
				axios.get(`/api/property/bis/illuminated-signs/${app.property.id}`)
					 .then((response) => { 
					 	app.isLoading()
					 })
