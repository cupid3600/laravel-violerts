<template>
	<div class="col" id="open_data_acris_ppl_col">
		<div class="m-portlet">
			<div class="m-portlet__head">
				<div class="m-portlet__head-caption">
					<div class="m-portlet__head-title">
						...
						<h3 class="m-portlet__head-text">
							ACRIS - Personal Property Legals
							<small>
								(Open Data)
							</small>
						</h3>
				...
				<div class="m-portlet__head-tools">
					<ul class="m-portlet__nav">
						...
						<li class="m-portlet__nav-item">
							<a @click="isOpen()" class="m-portlet__nav-link m-portlet__nav-link--icon" data-toggle="collapse" href="#OpenDataACRISPPL" role="button" aria-expanded="false" aria-controls="OpenDataACRISPPL">
			...
			<div class="m-portlet__body collapse table-responsive" id="OpenDataACRISPPL">
				...
				<vuetable
					...
					:api-url="`/property/open-data/acris-ppl/${property.id}`"
					:fields="fields"
			        :css="css.table"
			        :sortOrder="sortOrder"
				...
				<div id='open_data_acris_ppl_slider' class='swipe' v-else-if="panel.loading == 0 && isMobile">
...
<script>
	...
	import ACRISPPLDate from './Columns/PPLDate.vue'
    import Swipe from 'swipe-js'

	Vue.component('acris-ppl-date', ACRISPPLDate)
	...
	export default { 
		...
		data () { 
			return { 
				fields: [ 
					{ 
						name: '__component:acris-ppl-date', 
						sortField: '', 
						title: 'Date'
					},
					{ 
		...
		mounted() {
		    if (!this.isMobile) this.$events.$on('acris-ppl-filter-set', eventData => this.onFilterSet(eventData))
		    if (!this.isMobile) this.$events.$on('acris-ppl-filter-reset', e => this.onFilterReset())
		},
		watch: {
		...
		methods: {
            ...
            openMobilePanel () {
                ...
                document.getElementById('OpenDataACRISPPL').style = 'min-height: calc(100vh - 186.72px);'
                document.getElementById('open_data_acris_ppl_col').style = 'padding: 0px; position: fixed; top: 71.13px; height: 100%'
            ...
            closeMobilePanel () {
                ...
                document.getElementById('OpenDataACRISPPL').style = 'min-height: \'\';'
                document.getElementById('open_data_acris_ppl_col').style = 'padding: 15px; position: \'\'; top: \'\'; height: \'\';'
            ...
            applySwipe () {
                const swipeContainer = document.getElementById('open_data_acris_ppl_slider')
            ...
            getMobileData () {
                const app = this
                app.isLoading()
                axios.get(`/api/property/open-data/acris-ppl/swipe/${app.property.id}`)
                    .then((response) => {
			...
			getData () {
				var app = this
				app.isLoading()
				axios.get(`/api/property/open-data/acris-ppl/${app.property.id}`)
					 .then((response) => {
					 	app.isLoading()
					 })
            ...
            getMobileDataOnPageLoad () {
                axios.get(`/api/property/open-data/acris-ppl/swipe/${this.property.id}`)
                    .then((response) => {
                        this.panel.data = response.data
                    }).catch((error) => console.error(error))
            },
			updateVueTable () {
				...
				$('#OpenDataACRISPPL').collapse("hide")
			},
