<template>
	<div class="col" id="open_data_acris_rpl_col">
		<div class="m-portlet">
			<div class="m-portlet__head">
				...
				<div class="m-portlet__head-tools">
					<ul class="m-portlet__nav">
						...
						<li class="m-portlet__nav-item">
							<a @click="isOpen()" class="m-portlet__nav-link m-portlet__nav-link--icon" data-toggle="collapse" href="#OpenDataACRISRPL" role="button" aria-expanded="false" aria-controls="OpenDataACRISRPL">
			...
			<div class="m-portlet__body collapse table-responsive" id="OpenDataACRISRPL">
				...
				<vuetable
					...
					:api-url="`/property/open-data/acris-rpl/${property.id}`"
					:fields="fields"
			        :css="css.table"
			        :sortOrder="sortOrder"
				...
				<div id='open_data_acris_slider' class='swipe' v-else-if="panel.loading == 0 && isMobile">
...
<script>
	...
	import ACRISRPLDate from './Columns/RPLDate.vue'
	import ACRISRPLDocID from './Columns/RPLDocID.vue'
    import Swipe from 'swipe-js'

	Vue.component('acris-rpl-date', ACRISRPLDate)
	Vue.component('acris-rpl-doc-id', ACRISRPLDocID)
	...
	export default { 
		...
		data () { 
			return { 
				fields: [ 
					{ 
						name: '__component:acris-rpl-date', 
						sortField: '', 
						title: 'Date'
					},
					{ 
						name: '__component:acris-rpl-doc-id', 
						sortField: '', 
						title: 'Doc #'
					},
					{ 
			    ...
			    sortOrder: [
			        { name: 'document_id', sortField: 'document_id', direction: 'asc' },
			    ],
			    moreParams: {},
		...
		mounted() {
		    if (!this.isMobile) this.$events.$on('acris-rpl-filter-set', eventData => this.onFilterSet(eventData))
		    if (!this.isMobile) this.$events.$on('acris-rpl-filter-reset', e => this.onFilterReset())
		},
		watch: { 
		...
		methods: {
            ...
            openMobilePanel () {
                ...
                document.getElementById('OpenDataACRISRPL').style = 'min-height: calc(100vh - 186.72px);'
                document.getElementById('open_data_acris_rpl_col').style = 'padding: 0px; position: fixed; top: 71.13px; height: 100%'
            ...
            closeMobilePanel () {
                ...
                document.getElementById('OpenDataACRISRPL').style = 'min-height: \'\';'
                document.getElementById('open_data_acris_rpl_col').style = 'padding: 15px; position: \'\'; top: \'\'; height: \'\';'
                document.getElementById('row_parent').style = 'padding: 0px 30px;'
            },
            getMobileDataOnPageLoad () {
                axios.get(`/api/property/open-data/acris-rpl/swipe/${this.property.id}`)
                    .then((response) => {
            ...
            applySwipe () {
                const swipeContainer = document.getElementById('open_data_acris_slider')
            ...
            getMobileData () {
                const app = this
                app.isLoading()
                axios.get(`/api/property/open-data/acris-rpl/swipe/${app.property.id}`)
                    .then((response) => {
                        this.panel.data = response.data
                        app.isLoading()
			...
			getData () { 
				var app = this
				app.isLoading()
				axios.get(`/api/property/open-data/acris-rpl/${app.property.id}`)
					 .then((response) => { 
					 	app.isLoading()
					 })
			...
			updateVueTable () {
				...
				$('#OpenDataACRISRPL').collapse("hide")
			},
