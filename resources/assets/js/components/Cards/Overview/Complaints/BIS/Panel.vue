<template>
    <div class="col"  id="bis_complaints_col">
        <!-- print markup -->
        ...
        <div class="m-portlet d-print-none">
                    ...
                    <ul class="m-portlet__nav">
                        ...
                        <li class="m-portlet__nav-item">
                            <button class="m-portlet__nav-link btn btn-success btn-sm item-count-btn" @click="viewClosed(property.id)" :disabled="panel.loading == true">
                                <strong v-if="property.complaints_total != '' && property.complaints_open != ''">
                                    {{ (property.complaints_total - property.complaints_open) }} 
                                </strong>
                                <span>Resolved</span>
                            </button>
                        </li>
                        ...
                        <li class="m-portlet__nav-item">
                            <a @click="isOpen()" class="m-portlet__nav-link m-portlet__nav-link--icon" data-toggle="collapse" role="button" aria-expanded="false" aria-controls="BISComplaints" href="#BISComplaints">
            ...
            <div class="m-portlet__body collapse table-responsive" id="BISComplaints">
                ...
                <!--/desktop markup -->
                <!-- mobile (swipe) markup -->
                <div id='bis_complaints_slider' class='swipe' v-else-if="panel.loading == 0 && isMobile">
                    <div class='swipe-wrap'>
                        <div class='swipe-card' v-for="item in panel.data">
                            <div class="row">
                                <p class="col"><strong class="m--font-boldest">Issue Date</strong> {{ item.date_entered }}</p>
                                <p class="col"><strong class="m--font-boldest">Complaint #</strong> {{ item.complaint_number }}</p>
                            </div>
                            <div class="row">
                                <p class="col"><strong class="m--font-boldest">Category</strong> {{ item.category }}</p>
                                <p class="col"><strong class="m--font-boldest">Status</strong> {{ item.status }}</p>
                            </div>
                            <div class="row">
                                <p class="col"><strong class="m--font-boldest">Description</strong> {{ item.description }}</p>
                            </div>
                            <div class="row">
                                <p class="col"><strong class="m--font-boldest">Comments</strong> {{ item.comments }}</p>
                            </div>
                            <div class="row">
                                <div class="col">
...
<script>
    ...
    Vue.component('bis_comp_status', Status)
    Vue.component('bis_comp_view_more', ViewMore)
	...
	export default {
		...
		data () {
			return {
				fields: [
                    { 
                        name: '__component:bis_comp_view_more', 
                        sortField: '', 
                        title: 'View'
                    },
					{
					...
					},
					{
                        ...
                        headerClass: 'complaintHeader',
                        dataClass: 'complaintData',
						sortField: '',
						title: 'Complaint #'
					},
					{
					...
					{
						name: '__component:bis_comp_status',
						sortField: '',
						title: 'Status'
					},
				],
				css: {
			        table: {
			        	tableClass: 'table table-striped table-hover custom-panel bis-complaints',
		...
		mounted () {
            if (!this.isMobile) this.$events.$on('bis-comp-filter-set', eventData => this.onFilterSet(eventData))
            if (!this.isMobile) this.$events.$on('bis-comp-filter-reset', e => this.onFilterReset())
		},
		watch: {
			'property': 'loadApiUrl',
		...
		methods: {
            ...
            openMobilePanel () {
                ...
                document.getElementById('BISComplaints').style = 'min-height: calc(100vh - 186.72px);'
                document.getElementById('bis_complaints_col').style = 'padding: 0px; position: fixed; top: 71.13px; height: 100%'
            ...
            closeMobilePanel () {
                ...
                document.getElementById('BISComplaints').style = 'min-height: \'\';'
                document.getElementById('bis_complaints_col').style = 'padding: 15px; position: \'\'; top: \'\'; height: \'\';'
                document.getElementById('row_parent').style = 'padding: 0px 30px;'
            },
			loadApiUrl () {
				var app = this
				app.panel.api_url = `/property/bis/complaints/open/${this.property.id}`
			...
			updateVueTable () {
				...
				$('#BISComplaints').collapse("hide")
			},
            getMobileDataOnPageLoad () {
                axios.get(`/api/property/bis/complaints/swipe/${this.property.id}`)
                    .then((response) => {
			...
			viewOpen (id) {
				var app = this
				app.panel.api_url = `/property/bis/complaints/open/${id}`
				app.$refs.vuetable.refresh()
				app.panel.closed_items = 0
				app.panel.all_items = 0
				app.panel.open_items = 1
				$('#BISComplaints').collapse("show")
			},

			viewClosed (id) {
				var app = this
				app.panel.api_url = `/property/bis/complaints/closed/${id}`
				app.$refs.vuetable.refresh()
				app.open_items = 0
				app.closed_items = 0
			...
			viewAll (id) {
				var app = this
				app.panel.api_url = `/property/bis/complaints/all/${id}`
				app.$refs.vuetable.refresh()
				app.open_items = 0
				app.closed_items = 0
            ...
            applySwipe () {
                const swipeContainer = document.getElementById('bis_complaints_slider')
                window.bisComplaintsSwipe = new Swipe(swipeContainer, {
                    speed: 400,
                    disableScroll: true,
                    callback: (index, el) => {
                ...
                this.panel.swipe = window.bisComplaintsSwipe
...
<style>
    .bis-complaints .complaintHeader, .bis-complaints .complaintData { 
